...
- (BaseTableViewCell *)cellForRow:(int)row indexPath:(NSIndexPath *)indexPath tableView:(UITableView *)tableView {
    if (self.contact.user.pictureData) {
        [self.headerCell.pictureButton setImage:[UIImage imageWithData:self.contact.user.pictureData] forState:UIControlStateNormal];
        [self.headerCell.pictureButton addTarget:self action:@selector(viewPicture) forControlEvents:UIControlEventTouchUpInside];
    } else if ([self.contact.user.picture length]) {
        [[AsyncImageLoader sharedLoader] loadImageWithURL:[NSURL URLWithString:self.contact.user.picture] target:self action:@selector(imageLoaded:)];
    } else {
    ...
    self.headerCell.nameLabel.text = [self.contact.user formattedName];
    
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    [formatter setDateFormat:@"M/d h:mm a"];
    self.headerCell.timeLabel.text = [[formatter stringFromDate:self.contact.updatedAt] lowercaseString];
